"""Regenerate cover + inline images for the 2 paid notes (2026-05-14)
using the ChatGPT pipeline, then swap them in via edit_article.

Pre-req: Brave must be fully stopped OR CDP attach mode active
(CHATGPT_CDP_PORT=9222 with launch_brave_cdp.bat).
"""
...
logger = logging.getLogger("regen_paid")
...
def _kill_brave() -> None:
    """Only kill if CDP not configured. With CHATGPT_CDP_PORT set we attach."""
    if os.environ.get("CHATGPT_CDP_PORT"):
        logger.info("CDP port set; not killing Brave (attach mode)")
        return
    try:
        subprocess.run(
...
def _drop_local_image_lines(content: str) -> str:
    """Drop ``![alt](data/images/...)`` markdown so edit re-uploads fresh."""
    return re.sub(
        r"\n?!\[[^\]]*\]\(data/images/[^)\s]+(?:\s+\"[^\"]*\")?\)\n?",
        "\n",
        content,
...
def main() -> int:
    ...
    # Phase 1 — generate images via ChatGPT
    for j in jobs:
        logger.info("=" * 70)
        ...
        try:
            cover, inlines = chatgpt_image_batch(
                ...
                content=j["content"],
                inline_count=4,
                slug_hint=f"paidregen_{slug}",
                genre_hint=(
                    "tech business analysis / programming / AI infrastructure"
                ),
            )
        except Exception as exc:  # noqa: BLE001
        ...
        if cover:
            try:
                j["data"]["_cover_image_before_paidregen"] = (
                    j["data"].get("cover_image")
                )
                j["data"]["cover_image"] = str(cover.relative_to(_REPO))
                j["data"]["_inline_images_before_paidregen"] = (
                ...
                logger.warning(
                    "store write failed for %s: %s", j["aid"], exc,
                )
