...
- 横断パターン抽出を省略しない — これがRSI(自己改善)の主目的。単発記事の指摘だけでは
  呼び出し側は活用できない。
- **破壊的アクション(価格降格/非公開化/削除)は「提案」に留める** — あなたの判定を
  根拠に呼び出し側が即座に実収益を放棄しないよう、🔴判定でも「推奨: ¥0降格
  (要ユーザー承認)」の形式で書く。実行判断は人間 or 呼び出し側の確認プロセスに委ねる
  (2026-07-13 RSI process audit 指摘 d: レビュアー単一障害点の緩和)。
- **backlog 突き合わせ (毎回必須)**: レビュー開始時に
  `docs/knowledge/review_backlog.md` を読み、open 項目が今回のレビュー対象に
  関係するなら「解消/継続/悪化」を判定してレポート末尾に含める。新規の 🟡/NOTE
  指摘は backlog へ追記可能な1行フォーマット
  (`| # | 日付 | 対象 | 指摘 | 🟡 open |`) でも併記する。
