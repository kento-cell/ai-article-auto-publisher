...
- **7-28 routine (12日ぶり) + 文藝春秋SDGsエッセイ応募 + 夏の1コマ準備**:
  ...
  - **投稿後レビュー: CRITICAL 24 / WARN 16 / NOTE 7 (Slack通知済)**:
    - **事故 #27**: zenn scrap 2本が本文ゼロの空スクラップとして公開
      (35K字未投稿のまま成功扱い、7-16までは正常 → zenn UI 漂流疑い)。
      恒久対策 (投稿後検証の fail-closed 化) は未実装
    - ダングリング3変種で再発 (backlog#9 reopen→修復パス拡張済) /
      プロンプト転写 (H2./必須視覚要素N/カテゴリーN → sanitizer 3規則追加済) /
      BTS Jungkook 無ソース言及 / 色覚記事に法的基準の誤情報 /
      画像不一致 5/7本 / 実体験捏造2本 (backlog#24-30 追記、RAG 715 chunks)
    - **7-16対策の効果は verify 済**: 偽アンカー0・内部URI0・¥0降格2回正常
    - **明日publish予定のキャンプ記事を事前修正済** (store): 「プロが選ぶ」
      「ローカル限定」除去 + ダングリング8修復
    - **user 承認 → live修正 全4本 同日完了 (全live検証済)**:
      zenn 2本 = 本文クリーニング後 repost (add_post_to_scrap 新設、
      検証2/2成功、ops#27 恒久対策 = publish_scrap に live検証+リトライ+
      Slack通知も実装✅) / 色覚 = 誤情報をWCAG表現に訂正+空図見出し除去 /
      韓国ガジェット = BTS無ソース行+メタ漏れ削除
...
- **7-15 朝 routine (7-14対策の効果を live 実証 + carryover 穴 #26 検出)**:
  ...
  - **user 判断=本文補完+タイトル修正 (¥500維持) → 同日全実施**:
    - live 4本修正 (`_fix_articles_20260715.py`、全live検証済): ベビーモニター
      (まとめ+最終判定表+AAP無出典除去+価格注記)、猛暑対策 (タイトルから
      専門家/氷点下除去+年ずれ修正)、ブロワー (ダングリング6修復+チェック
      リスト補完)、エクソソーム (偽アンカーリンク3+架空オファー除去)
    - **恒久対策も同日実装 (ops#26 → ✅)**: publish時 re-sanitize パス
      (在庫の防御バージョン乖離を構造吸収) / 長文リスト項目切断検知 /
      タイトル名詞約束チェック2層 / 実URL引用ゼロ×有料→¥0強制 /
      偽アンカーリンク line-kill。regression 85ケース全PASS、RAG 700 chunks
