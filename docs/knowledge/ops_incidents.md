...
## 27. zenn scrap の本文コメント投稿が silent 失敗 → 空スクラップ公開

**事象:** 2026-07-28 publish の zenn scrap 2本 (Staging環境/レガシー複雑性) が
本文ゼロの空スクラップとして公開された (commentsCount=0 を live 確認)。
スクラップ作成とタイトル設定は成功し、本文コメントの投稿だけが失敗したが
エラーにならず、published_url/published_at が書き戻されて成功扱いに。
