...
# 2026-06-10: 事実主張型ブラケットを除去 (100人に聞いた/現地レポ/リーク/
# プロが教える/現場の声)。これらは本文に裏付け (実際の調査・現地取材・
# 取材源) が存在せず、煽り (合法グレー) ではなくタイトル捏造になる。
# 【100人に聞いた】が zenn scrap fb51639f490b4a に公開された実害事故対応。
# 残したものは純粋な誇張・感情表現 (号泣/衝撃/保存版 等) のみ。
...
def _filter_anti_pattern_titles(ranked: dict) -> dict:
    """Drop articles whose source title carries a learned anti-pattern prefix.

    Anti-patterns are read at runtime from
    ``docs/knowledge/quality_anti_patterns.md`` (auto-regenerated by
    ``scripts/analyze_performance.py``). Filtering at the collector-output
    stage keeps the Writer from picking up the doomed angle in its title.
    The Writer can still produce its own title, but skipping doomed source
    seeds at this layer is a cheap, high-leverage pre-cut.
    """
    try:
        from learners.prompt_updater import load_anti_pattern_title_prefixes
    except Exception as exc:
        logger.debug("[anti-pattern] loader import failed: %s", exc)
        return ranked

    prefixes = load_anti_pattern_title_prefixes()
    if not prefixes:
        return ranked
    ...
    for platform, articles in ranked.items():
        ...
        if dropped:
            logger.info(
                "[anti-pattern] %s: dropped %d candidate(s) matching %s",
                platform, dropped, prefixes,
            )
        filtered[platform] = kept
    return filtered
...
def publish_approved(
    ...
    _PUBLISH_DENY_PATTERNS = [
        # 2026-06-10 タイトル捏造事故: 旧 _TITLE_BRACKETS 由来の
        # 「【100人に聞いた】」が本文 H1 → 日本語タイトル抽出 経由で
        # zenn scrap に公開された。アンケート実施の事実がない記事で
...
def _run_pipeline_inner(config: dict, prompts: dict, mode: str):
    ...
    if mode == "generate":
        ...
        ranked = _filter_anti_pattern_titles(ranked)
